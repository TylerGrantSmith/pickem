...
library(pickem)
library(data.table)
input_file <- here::here("input","NFL Pool.xlsm")
results <- get_results(input_file, 17)
knitr::kable(results)
```
...
probs[outcomes[ranks[RANK==1], on = c("SIM_ID", "WEEK", "SCENARIO_PROB")], on = c("WEEK", "GAME_ID", "HOME_OR_AWAY" = "OUTCOME")][,.(.N,WIN_SCENARIO_PERCENT = scales::percent(.N/num_win_scenarios, 0.01)),TEAM][order(-N)]
